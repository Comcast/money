...
/**
 * [[StreamTracingDSL]] are used to trace a Akka Stream that has not been built to support tracing
 *
...
object StreamTracingDSL {
  ...
  implicit class PortOpsSpanInjector[In: ClassTag](portOps: PortOps[(In, SpanContextWithStack)])(implicit
    ...
    /**
     * Completes the Streams Span in the SpanContextWithStack this ends the tracing of the stream
     *
     * Example:
     *
     * {{{
     ...
     * }}}
     *
     * @param sink the sink that you wish to end the stream with
     * @tparam T must be covariant on [[In]] ie must either be a supertype or the type of the output of the stream
     */

    def ~|[T >: In](sink: SinkShape[T]): Unit = completeTracing(sink.in)
